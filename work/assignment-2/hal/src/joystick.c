...
static void* read_thread(void* arg)
{
    ...
    while (!joystick->stop_thread) {
        ...
        write_i2c_reg16(joystick->i2c_file_desc, REG_CONFIGURATION, TLA2024_CHANNEL_CONF_LIGHT);
        usleep(I2C_UPDATE_DELAY);

        uint16_t raw_light = read_i2c_reg16(joystick->i2c_file_desc, REG_DATA);
        uint16_t light_value = ((raw_light & 0x0FF) << 8) | ((raw_light & 0xF00) >> 8);
        light_value = light_value >> 4;

        pthread_mutex_lock(&joystick->lock);
        {
            joystick->light_value = light_value;
        }
        pthread_mutex_unlock(&joystick->lock);
...
float Joystick_read_light(Joystick *joystick)
{
    assert(joystick != NULL);
    pthread_mutex_lock(&joystick->lock);
    float value = joystick->light_value;
    pthread_mutex_unlock(&joystick->lock);
    return value;
}
